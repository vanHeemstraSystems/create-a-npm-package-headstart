...
i agree with you so that's the reason that we have another tool called cometizen right i never know how to type communism so google will help me out and when you
have communism you can go over here and set up committees and and committees and will open a window like this every time that you do get cz
it will open a very very nice window so it will help you with your commits and you don't need to know all of those tedious messages out of your head and so let's
go over here to npx and they have a way for us to set it up which is over here npx comet is an init and we will just copy that and you will see that
as soon as i copy that and let it run which will be another two or three minutes so another fast forward but as soon as that finishes we will be able to go to our
package.json and add over here into our package.json i'm sorry over here into our package.json another command called commit and that commit we'll just do git cc
